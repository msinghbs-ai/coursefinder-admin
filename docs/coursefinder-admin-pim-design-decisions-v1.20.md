...
## Decision 25 — international recruitment contacts are Provider decision context

Provider detail may surface public professional contacts relevant to international recruitment, admissions, regional management, country/market management and international student engagement.

The routine Provider blade uses a dedicated **International contacts** decision section rather than exposing raw contact tables.

Presentation must distinguish:
- **First-party university** — preferred; exact university-published assignment/contact;
- **Licensed enrichment** — secondary professional context;
- **Governed manual** — staff-resolved with provenance.

Each visible contact should show, where available:
- name/team;
- title;
- territory/market assignment;
- institutional work email/phone;
- source/provider;
- university/professional profile link;
- verification/freshness;
- Evidence for first-party observations.

A Provider may have multiple rows for the same person when the university publishes different territory-specific aliases/emails. The UI must not collapse materially distinct country/market assignments simply because the manager name is the same.

Licensed enrichment does not overwrite university-published values. A mismatch is a change/review signal, not an automatic replacement.

Initial discovery should not flood the operator with “new contact” alerts. The alert surface is reserved for meaningful subsequent title, territory, contact, removal or restoration changes.
