# CourseFinder Data Operations Admin Guide v1.6

**Status:** CURRENT M2.4 DATA OPERATIONS GUIDE  
**Date:** 30 August 2026  
**Supersedes:** `docs/coursefinder-m2-4-data-operations-admin-guide-v1.5.md`  
**Change Controls:** prior accepted controls plus `CF-CHG-20260829-046`, `CF-CHG-20260829-047` and active `CF-CHG-20260830-048`  
**Pilot UI:** PIM Admin v2.15.10 and later until superseded
...
## 21. A14 telemetry and provider economics

Layer 2 and Layer 3 operational telemetry is retained continuously under Execution Addendum A14.

Layer 2 reporting distinguishes acquisition-provider attempts from managed-run execution. Preserve provider, immutable source profile/version, latency, retries, routing/fallback outcome, vendor units/quota, cost, Evidence footprint, fields resolved and Layer 3 fall-out. Direct HTTP has zero vendor units; subscription-backed providers are not assumed free merely because retained cash cost is zero.

Layer 3 reporting preserves exact configured/returned model, task/prompt profile, external-call count, input/output tokens, latency, validator result and cost when returned by the provider/runtime. Missing token/cost usage stays explicit unavailable rather than inferred.

The initial baseline is retained in `project-runsheets/milestone-2/m2.4/m2.4.2/TELEMETRY-BASELINE.md`.


## 17. Provider international contact intelligence — A15

Provider detail now includes **International contacts** when governed contact observations exist.

### What the section means

Contacts are Provider/institute context, not Course identity facts.

Source badges mean:
- **First-party university** — published on a governed university/institute website and preferred;
- **Licensed enrichment** — professional context from an authorised data provider; secondary;
- **Governed manual** — staff-resolved observation with provenance.

### What operators should validate

For first-party contact rows, verify:
1. contact/manager name or team;
2. international recruitment/admissions/regional role;
3. territory/market assignment where published;
4. institutional work email/phone;
5. source link;
6. Evidence;
7. last verification date.

The same manager may legitimately appear more than once when the university publishes separate territory email aliases. Do not merge materially different territory assignments solely because the name is identical.

### Contact discovery status

A completed university check with zero verified rows is valid. It means no contact met the deterministic A15 acceptance rules on the checked sources; it is not proof the university has no international staff.

Low-confidence/generic page text is retained historically as rejected where appropriate and must not be promoted just to improve coverage.

### Licensed enrichment

Pilot includes an Apollo domain/title adapter, but it is optional. The adapter:
- searches professional roles by university domain/title;
- does not request personal email reveal;
- does not request phone reveal;
- cannot overwrite a first-party university contact;
- records provider/call/latency/unit/cost telemetry where available.

If the provider credential is absent, display/configuration remains blocked without affecting first-party contact discovery.

### Privacy and authority

Do not copy private/personal contact details from unrelated sources into A15. The normal target is public professional/institutional contact information.

Contact intelligence does not:
- change Layer 1 Provider/Course identity;
...
## 22. M2.4.4 cross-layer operations

The normal operator model remains layer-specific even though scheduling and housekeeping share programme governance.

**Layer 1:** authoritative verification/recovery only. A legacy `regulatory_sync` job older than 45 minutes is recoverable only when it is not owned by a current Layer 1 run heartbeat.

**Layer 2:** deterministic provider-attempt, Job and managed-batch recovery. Provider timeouts and batch execution policy control the stale window.

**Layer 3:** governed Evidence interpretation only. A stale `reserved`/`calling` interpretation older than 20 minutes becomes a recoverable provider error with provenance; it does not become canonical truth.

### Scheduling

The general refresh scheduler creates bounded requests and deduplicates active targets. Layer 1 and Layer 2 schedulers then own their layer-specific dispatch/reconciliation. Do not manually enqueue a second refresh because an existing request is slow; inspect its governed state first.

### Operational alerts

Layer 1 health exposes source freshness, variance, validation, heartbeat and schedule failures. Layer 2 alerts expose stale runs, paused profiles, blocked items, provider failure streaks and quota reserve.

Layer 3 now has a governed rank-4+ alert read for stale execution, profile benchmark/state problems, repeated provider errors and recorded cost-ceiling breaches. A healthy checkpoint may legitimately return no Layer 3 alerts.

### Storage

Evidence object count/bytes are operational signals, not an implicit deletion or quota rule. M2.4.4 observed 6,248 private Evidence objects using 3,781,700,044 bytes. No governed storage threshold is currently configured, so the Admin must not claim a fabricated limit.

### Telemetry

A14 remains mandatory on active Layer 2/3 execution paths. Missing vendor units, tokens or cost remain unavailable rather than being backfilled by estimation unless the governed provider/runtime supplies an accepted basis.

### Migration alias

The deployed Layer 1 recovery history version is `20260830021159`; the prior repository mirror filename is `20260830021400...`. Their function body is reconciled. Do not rerun the migration to make filenames cosmetically identical.
