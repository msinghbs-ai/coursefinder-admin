# CourseFinder PIM Admin Guide v1.23

**Status:** CURRENT M2.5 PILOT ADMIN GUIDE  
**Date:** 30 August 2026  
**Supersedes:** `docs/coursefinder-pim-admin-guide-v1.22.md`  
...
## M2.5 Layer 3 Provider source-pattern operations — CF-054

This workflow is a **manual-governed source-resolution path**, not an autonomous AI pipeline.

### When it appears

Layer 2 Provider qualification can route a Provider to Layer 3 when deterministic Evidence supports possible Course/catalogue links but the Layer 2 identity controls cannot safely qualify the source pattern.

In Layer 3 — AI Interpretation, the Provider appears in **Governed Provider source-pattern queue** when:
- the refresh request is `requested_layer=3`;
- the reference is `A23-SOURCE-PATTERN:*`;
- retained governed Evidence exists;
- `openrouter-source-pattern-v1` is enabled, unpaused and benchmark-PASS.

### Operator action

1. Open **Layer 3 — AI Interpretation**.
2. Review the oldest Provider source-pattern requests first.
3. Confirm Provider, country, source URL and Evidence are plausible for the intended institution.
4. Use **Open source** if a first-party page check is required.
5. Select **Run source-pattern interpretation** for one Provider only.

There is deliberately no **Run all** action.

The signed-in user must hold Curator rank or higher. The browser does not choose the Evidence ID, Provider ID, model profile or revalidation lineage for this path; the server derives them from the governed request.

### What the AI is allowed to do

The model may return:
- one exact HTTPS URL already present in the retained same-host Evidence link set; or
- no candidate.

The model may not:
- manufacture or normalise a URL;
- infer or redefine Provider/Course regulatory identity;
- qualify the Provider directly;
- change Course fees, intakes, descriptions or identity;
- write Search or Publication state.

### What happens next

**Valid URL candidate**
→ returned to Layer 2  
→ new qualification-candidate source-profile version  
→ existing three Course identity controls dispatched  
→ normal deterministic Course discovery + CRICOS/detail verification  
→ only the Layer 2 reconciler can later qualify the Provider.

**No candidate / low confidence**
→ Provider moves to Layer 4 source resolution  
→ human verifies/corrects the first-party Course source.

**Provider/model/validation error**
→ request stays visible/retryable with the last attempt reason.

### Current Pilot boundary

At the 1 September 2026 CF-054 checkpoint:
- source queue: 390 queued;
- real source-pattern interpretations executed under CF-054: 0;
- queue was not bulk-drained;
- model profile limit remains 10/minute and 30/day;
- Admin source is v2.15.17 but deployed Cloudflare Pilot UI is blocked by external Git deployment drift.

Until that deployment is reconciled, do not interpret a green source-contract CI run as proof that the browser Worker contains this queue.

### Troubleshooting

If the source-pattern queue is missing in the deployed browser while repository `main` contains it:
- do not weaken role/security checks;
- do not restore legacy RPCs;
- do not manually rewrite the refresh requests;
- reconcile the Cloudflare external Git deployment for Worker `coursefinder-pilot`;
- rerun CF-053 and CF-054 deployed browser UAT unchanged.
