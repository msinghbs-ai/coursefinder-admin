...
| M245-FU-040 | CF-093 / reconstruction P1 | Immutable applied `20260912005948` fails fresh replay because UQ foundation lacks `discovery_strategy` | **P1 / SOURCE REMEDY PREPARED / HISTORY SYNC PENDING** | Bootstrap `20260912005000` now precedes `005948`; authorised CLI must run `supabase migration repair 20260912005000 --status applied --linked`, then prove migration-list parity and fresh db reset. Do not directly edit migration-history table. |
| M245-FU-034 | CF-093 / Codex | Nine findings returned; eight forward-fixed/resolved; reconstruction fix now prepared on head `013b2878...` | **ACTIVE / REVIEW REQUIRED** | Obtain fresh exact-head Codex review; do not merge on prior review evidence. |
...
| M245-FU-043 | CF-093 / migration reconstruction proof | Current connector cannot execute official Supabase `migration repair`; official docs support it for tracking-only repair when schema effect already exists | **AUTHORISED OPERATOR ACTION PENDING** | Run official CLI repair, then `supabase migration list` and fresh `supabase db reset`; record evidence. Direct SQL history mutation prohibited. |
| M245-FU-035 | CF-093 / Layer 3 | Qualified JWT-protected contract inspected; 53 historical L3-required Evidence items at inspection | **PAUSED BEHIND CF-093 RECOVERY** | Resume bounded authenticated live-provider acceptance only after reconstruction and corrective discovery close. |
...
1. Fresh exact-head Codex review for `013b2878...`.
2. Official Supabase history repair for retroactive bootstrap + migration-list parity + fresh reconstruction proof.
