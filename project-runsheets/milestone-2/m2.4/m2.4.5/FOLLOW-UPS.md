...
|---|---|---|---|---|
| M245-FU-032 | H4 / CF-CHG-20260910-093 | Scheduled Tasks operator maturity + governed AU Course Facts target builder + runtime observability/efficiency | REOPENED / RUNTIME OBSERVABILITY DEPLOYED / CONSEQUENTIAL ACCEPTANCE BLOCKED | Pilot PR #79 head `849f6f50e810bd77a6b3e02ef765df9c43f541db` has exact-head Fresh Reconstruction `34751520430` PASS, Targeted Recovery `34751520442` PASS, Frontend Build `34751520437` PASS, Cloudflare PASS and current Gitar findings resolved. Pilot runtime has forward-only terminal-timestamp migration `20260913100615` (repo source `20260913101000...`) and governed `jobs_runtime` metrics migration `20260913102217` (repo source `20260913102500...`). Rank-4 read ACL and safe-field boundary are proven; recent-50 wrapper measured ~21.4 ms. No natural discovery run has occurred after the timestamp correction, so post-fix discovery throughput is not yet observed. Provider health remains active bottleneck: scrape-do 401 and ScraperAPI credential-unavailable evidence must be remediated through governed provider/secret lifecycle; do not weaken fallback/security. Next action: observe legitimate natural post-fix discovery, accumulate comparable metrics, remediate provider health, then tune only from Before → Change → After evidence. Separate CF-093 closure still requires a legitimate current-main policy-qualified consequential Preview → dispatch → dedupe/Jobs/Evidence acceptance. |
| M245-FU-031 | H4 / CF-092 | Scheduled Tasks configuration, audited schedule editing and bounded Layer 1–2 run control | CLOSED / PASS | PR 66 to Pilot 9305eb3a004d12724ec26b6bab65e1d4b1ab2239; v2.15.76; preview 34468684736 PASS; post-merge build 34470101950 PASS; deployed UAT 34470101936 attempt 2 PASS after retained transient attempt-1 CF-102 failure. |
...
## Priority reconciliation — 13 September 2026

| Priority | Workstream | Immediate action | Status |
|---|---|---|---|
| 1 | CF-CHG-20260910-093 runtime operations + consequential acceptance | Observe legitimate natural post-fix discovery and collect `jobs_runtime` performance/Evidence/failure-class history; remediate provider credential health without weakening 401/fallback; tune only from evidence. Keep consequential acceptance separate until a genuinely governed policy-qualified target exists. | ACTIVE / ACCEPTANCE BLOCKER |
