...
| M25-FU-015 | Pilot Cloudflare external Git deployment drift | Current `Coursefinder-Pilot/main` v2.15.17 is not being published to `coursefinder-pilot.techm.workers.dev`; CF-053 browser acceptance fails against the stale bundle | Reconcile Cloudflare Worker `coursefinder-pilot` Git integration against `msinghbs-ai/Coursefinder-Pilot` / `main`, build/deploy settings and latest deployment; then rerun CF-053 + CF-054 deployed browser gates unchanged | BLOCKED / EXTERNAL CLOUDFLARE CONTROL PLANE |
| M25-FU-016 | Layer 3 source-pattern operator execution | CF-054 backend/Edge/manual queue implemented; legacy HTTP→HTTPS same-host hand-back reconciled; rollback paths PASS; first post-reconcile CI failed only on test syntax | Check clean rerun trigger `1605d15b...`; after FU-015 deployment reconciliation browser-prove v2.15.17 queue; real operator canary remains separately bounded | IMPLEMENTED / SOURCE+ROLLBACK PASS / CLEAN TARGETED RERUN PENDING / DEPLOYED UI BLOCKED |
