...
| M25-FU-015 | Pilot Cloudflare external Git deployment drift | Prior CF-053 browser checks saw a stale Worker bundle | Combined non-mutating CF-053/054 currentness gate triggered by `e15aee26...`; check its result first. If it still fails on missing current UI elements, reconcile Cloudflare Worker `coursefinder-pilot` Git integration against `msinghbs-ai/Coursefinder-Pilot` / `main` | UAT RUN PENDING / EXTERNAL CLOUDFLARE IF FAIL |
| M25-FU-016 | Layer 3 source-pattern operator execution | CF-054 backend/Edge/manual queue implemented; legacy HTTP→HTTPS same-host hand-back reconciled; rollback paths PASS; source-contract run `33492875364` PASS | Combined deployed browser proof triggered by `e15aee26...`; do not perform a real model canary unless separately authorised after UI proof | IMPLEMENTED / SOURCE+ROLLBACK TARGETED PASS / DEPLOYED UI RUN PENDING |
