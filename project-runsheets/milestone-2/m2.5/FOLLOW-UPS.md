...
| M25-FU-050 | Legacy/merged Provider mapping | Legacy/current labels can create duplicates if matched naively | Deterministic catalogue/alias mapping plus explicit contact crosswalks are implemented; Victoria University remains a governed two-CRICOS ambiguity | IMPLEMENTED / 1 AMBIGUITY OPEN |
| M25-FU-051 | Provider Contacts Catalogue module | Dedicated database-management surface required | Catalogue route, paged searchable/filterable/sortable grid, column controls, row drawer and Provider deep-links are implemented in v2.15.41 | IMPLEMENTED / TARGETED UAT ACTIVE |
| M25-FU-052 | Individual edit/delete/restore/history | Managed lifecycle required | Rank-5 create/edit/verify, append-only versions, soft-delete, restore, status filtering and audit are implemented | IMPLEMENTED / TARGETED UAT ACTIVE |
| M25-FU-053 | Contact export / audit / CSV safety | Governed module-level export required | Filtered-view export, selected-column handling, audit and spreadsheet formula-injection protection are implemented | IMPLEMENTED / TARGETED UAT ACTIVE |
| M25-FU-054 | A30 acceptance / consumer boundary | New PIM surface could regress A15 privacy/Evidence or leak into consumers | Security/ACL checks pass; targeted deployed browser UAT candidate 1dbfef925c576e061dcb72f347487615548d0e2c is active; consumer admission remains unchanged | TARGETED UAT ACTIVE / CF-080 |
