-- M1-PIM-FINALISATION Course list performance gate
-- Filter/count/sort lightweight canonical rows first; enrich only the bounded page.
-- Derived catalogue-wide filters/sorts retain the accepted legacy implementation.

create index if not exists course_fees_admin_registered_latest_idx
  on catalogue.course_fees(course_id,source_snapshot_at desc,last_verified_at desc,created_at desc)
  include(amount,currency_code)
  where fee_type='tuition' and basis='registered_total_course' and coalesce(status,'active')='active';
...
declare
  ...
  v_requires_derived_filter boolean;
  v_result jsonb;
begin
  ...
  v_requires_derived_filter :=
    v_has_fee is not null or v_has_intake is not null or v_has_english is not null or
    v_has_scholarship is not null or v_has_state is not null or v_has_link is not null or
    nullif(p_args->>'min_completeness','') is not null or v_sort in ('fee','completeness');

  if v_requires_derived_filter then
  ...
  with base as (
    select c.id,c.stable_key,c.canonical_title,c.display_title,c.course_code,c.course_url,
    ...
    where (nullif(trim(coalesce(p_args->>'query','')),'') is null
      ...
      and (nullif(trim(coalesce(p_args->>'subdivision_code','')),'') is null or exists(
        select 1 from catalogue.course_campuses cc join catalogue.campuses ca on ca.id=cc.campus_id join ref.subdivisions sd on sd.id=ca.subdivision_id
    ...
    select pg.id,pg.stable_key,pg.canonical_title,pg.display_title,pg.course_code,pg.course_url,
    ...
    cross join lateral (
      select exists(select 1 from catalogue.course_registrations r where r.course_id=pg.id) has_registration,
  ...
  select jsonb_build_object(
    ...
    'execution_profile','paged_enrichment_v2'
  ) into v_result from enriched e;
  return v_result;
end
$$;

revoke execute on function security.admin_course_page_fast(jsonb) from public, anon;
